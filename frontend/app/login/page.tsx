...
// We create an inner component so we can use the `useSearchParams` hook,
// which must be wrapped in a <Suspense> boundary.
function LoginComponent() {
  ...
  // This useEffect handles the interactive mouse-tracking glow effect.
  useEffect(() => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // The AuthContext handles redirection on success
    } catch (err: any) {
    ...
    <div className="auth-container">
      <div className="auth-card" ref={cardRef}>
        <div className="card-content">
          <h2>Welcome Back</h2>
          <p className="subtitle">Log in to unlock your potential.</p>
          
          <form onSubmit={handleSubmit}>
            {error && <p className="message error-text">{error}</p>}
            {registrationSuccess && <p className="message success-text">Registration successful! Please log in.</p>}
            ...
            <button type="submit" disabled={loading}>
              {loading ? <Spinner /> : 'Login Securely'}
            </button>
          </form>
...
// The main export for the page wraps the component in Suspense
    ...
    <>
      ...
      <style jsx global>{`
        ...
        .auth-container { 
          ...
          background-image: 
            radial-gradient(circle at 10% 20%, hsla(210, 30%, 80%, 0.3), transparent 50%),
            radial-gradient(circle at 80% 90%, hsla(210, 30%, 80%, 0.3), transparent 50%);
          overflow: hidden;
        }
        .auth-card { 
          position: relative;
        ...
        .success-text { color: #155724; background-color: #d4edda; }
